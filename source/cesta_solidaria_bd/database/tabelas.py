...
# ----------------------------------------------------- PESSOAS/ENTIDADES -----------------------------------------------------
        ...
        self.agente = Table('agentes', self.metadata,
            ...
            Column('tel_contato', String, nullable=False)
        )

        self.fornecedor = Table('fornecedores', self.metadata,
            Column('id', Integer, primary_key=True),
            Column('CNPJ', String, unique=True, nullable=False),
            Column('tipo', String, nullable=False), # 'EMPRESA PRIVADA', 'EMPRESA ESTATAL', 'ONG'
            Column('email', String, unique=True, nullable=False),
            Column('tel_contato', String, unique=True, nullable=False)
        )
...
# -------------------------------------------------- ESTRATÉGIA E SERVIÇOS ---------------------------------------------------
        ...
        self.lote = Table('lotes', self.metadata,
            Column('id', Integer, primary_key=True),
            Column('vulnerabilidade_id', Integer, ForeignKey('vulnerabilidades.id'), unique=True, nullable=False),
            Column('etiqueta', String, unique=True, nullable=False),
        ...
        self.cesta = Table('cestas', self.metadata,
            ...
            Column('status', String, nullable=False), # 'DISPONIVEL', 'AVARIADA', 'ENTREGUE'
...
# --------------------------------------- TABELAS DE RELACIONAMENTOS OPCIONAIS (0:N) ----------------------------------------
        ...
        self.deficiencia = Table('deficiencias', self.metadata,
            ...
            Column('cod_deficiencia', String, nullable=False),
        ...
        self.alteracao = Table('alteracoes', self.metadata,
            ...
            Column('tipo', String, nullable=False), # 'AGENTE NÃO ATENDIDO', 'ROTA COMPROMETIDA', 'CESTA DANIFICADA'
            Column('descricao', Text, nullable=False),
...
# ------------------------------------------------ TABELAS RELACIONAIS (N:N) -------------------------------------------------
        ...
        self.entrega = Table('entregas', self.metadata,
            ...
            Column('assinatura_agente', String, nullable=False),
            Column('assinatura_destinatario', String, nullable=False)
        )
